var sip = require ('../index.js')
var Zeq = require('@mayama/zeq')
var z = new Zeq()
var m = require('data-matching')
var sip_msg = require('sip-matching')
var sdp = require('sdp-matching')

async function test() {
    await sip.set_log_level(9)

    //await sip.set_log_level(6)
    sip.dtmf_aggregation_on(500)

    // Let's ignore '100 Trying'
    z.add_event_filter({
        event: 'response',
        msg: sip_msg({
            $rs: '100',
        }),
    })

    z.trap_events(sip.event_source, 'event', (evt) => {
        var e = evt.args[0]
        return e
    })

    console.log(await sip.start((data) => { console.log(data)} ))

    t1 = await sip.transport.create({address: "127.0.0.1", type: 'udp'})
    t2 = await sip.transport.create({address: "127.0.0.1", type: 'udp'})

    console.log("t1", t1)
    console.log("t2", t2)

    oc = await sip.call.create(t1.id, {from_uri: 'sip:alice@test.com', to_uri: `sip:bob@${t2.address}:${t2.port}`, media: 'audio,audio'})

    await z.wait([
        {
            event: "incoming_call",
            call_id: m.collect("call_id"),
        },
    ], 1000)

    ic = {
        id: z.$call_id,
        sip_call_id: z.$sip_call_id,
    }

    await sip.call.respond(ic.id, {code: 183, reason: 'Session Progress', media: 'audio,audio'})

    await z.wait([
        {
            event: 'response',
            call_id: oc.id,
            method: 'INVITE',
            msg: sip_msg({
                $rs: '183',
                $rr: 'Session Progress',
            }),
        },
        {
            event: 'media_update',
            call_id: oc.id,
            status: 'ok',
            media: [
              {
                type: 'audio',
              },
              {
                type: 'audio',
              },
           ],
        },
        {
            event: 'media_update',
            call_id: ic.id,
            status: 'ok',
            media: [
              {
                type: 'audio',
              },
              {
                type: 'audio',
              },
            ],
        },
    ], 1000)

    await sip.call.start_inband_dtmf_detection(oc.id)

    await sip.call.send_dtmf(oc.id, {digits: '1234', mode: 0})
    await sip.call.send_dtmf(ic.id, {digits: '4321', mode: 1})

    await z.wait([
        {
            event: 'dtmf',
            call_id: ic.id,
            digits: '1234',
            mode: 0,
            media_id: 0
        },
        {
            event: 'dtmf',
            call_id: ic.id,
            digits: '1234',
            mode: 0,
            media_id: 1
        },
        {
            event: 'dtmf',
            call_id: oc.id,
            digits: '4321',
            mode: 1,
            media_id: 0
        },
	{
            event: 'dtmf',
            call_id: oc.id,
            digits: '4321',
            mode: 1,
            media_id: 1
        },
    ], 2000)

    await z.sleep(1000)

    await sip.call.respond(ic.id, {code: 200, reason: 'OK', media: 'audio,audio'})

    await z.wait([
        {
            event: 'response',
            call_id: oc.id,
            method: 'INVITE',
            msg: sip_msg({
                $rs: '200',
                $rr: 'OK',
            }),
        },
    ], 1000)
 
    await sip.call.send_dtmf(oc.id, {digits: '1234', mode: 0})
    await sip.call.send_dtmf(ic.id, {digits: '4321', mode: 1})

    await z.wait([
        {
            event: 'media_update',
            call_id: oc.id,
            status: 'ok',
            media: [
              {
                type: 'audio',
              },
              {
                type: 'audio',
              },
            ]
        },
        {
            event: 'dtmf',
            call_id: ic.id,
            digits: '1234',
            mode: 0,
            media_id: 0
        },
        {
            event: 'dtmf',
            call_id: ic.id,
            digits: '1234',
            mode: 0,
            media_id: 1
        },
        {
            event: 'dtmf',
            call_id: oc.id,
            digits: '4321',
            mode: 1,
            media_id: 0
        },
        {
            event: 'dtmf',
            call_id: oc.id,
            digits: '4321',
            mode: 1,
            media_id: 1
        },
    ], 2000)

    await sip.call.reinvite(oc.id, {media: 'audio,audio'})

    await z.wait([
        {
            event: 'reinvite',
            call_id: ic.id,
        },
    ], 1000)

    await sip.call.respond(ic.id, {code: 200, reason: 'OK', media: 'audio,audio'})

    await z.wait([
        {
            event: 'response',
            call_id: oc.id,
            method: 'INVITE',
            msg: sip_msg({
                $rs: '200',
            }),
        },
        {
            event: 'media_update',
            call_id: ic.id,
            status: 'ok',
            media: [
              {
                type: 'audio',
              },
              {
                type: 'audio',
              },
            ]
        },
        {
            event: 'media_update',
            call_id: oc.id,
            status: 'ok',
            media: [
              {
                type: 'audio',
              },
              {
                type: 'audio',
              },
            ]
        },
    ], 1000)

    await sip.call.send_dtmf(oc.id, {digits: '1234', mode: 0})
    await sip.call.send_dtmf(ic.id, {digits: '4321', mode: 1})

    await z.wait([
        {
            event: 'dtmf',
            call_id: ic.id,
            digits: '1234',
            mode: 0,
            media_id: 0
        },
        {
            event: 'dtmf',
            call_id: ic.id,
            digits: '1234',
            mode: 0,
            media_id: 1
        },
        {
            event: 'dtmf',
            call_id: oc.id,
            digits: '4321',
            mode: 1,
            media_id: 0
        },
        {
            event: 'dtmf',
            call_id: oc.id,
            digits: '4321',
            mode: 1,
            media_id: 1
        },
    ], 2000)

    await sip.call.reinvite(ic.id, {media: 'audio,audio'})

    await z.wait([
        {
            event: 'reinvite',
            call_id: oc.id,
        },
    ], 1000)

    await sip.call.respond(oc.id, {code: 200, reason: 'OK', media: 'audio,audio'})

    await z.wait([
        {
            event: 'response',
            call_id: ic.id,
            method: 'INVITE',
            msg: sip_msg({
                $rs: '200',
            }),
        },
        {
            event: 'media_update',
            call_id: oc.id,
            status: 'ok',
            media: [
              {
                type: 'audio',
              },
              {
                type: 'audio',
              },
            ]
        },
        {
            event: 'media_update',
            call_id: ic.id,
            status: 'ok',
            media: [
              {
                type: 'audio',
              },
              {
                type: 'audio',
              },
            ]
        },
    ], 1000)

    await sip.call.send_dtmf(oc.id, {digits: '1234', mode: 0})
    await sip.call.send_dtmf(ic.id, {digits: '4321', mode: 1})

    await z.wait([
        {
            event: 'dtmf',
            call_id: ic.id,
            digits: '1234',
            mode: 0,
            media_id: 0
        },
        {
            event: 'dtmf',
            call_id: ic.id,
            digits: '1234',
            mode: 0,
            media_id: 1
        },
        {
            event: 'dtmf',
            call_id: oc.id,
            digits: '4321',
            mode: 1,
            media_id: 0
        },
        {
            event: 'dtmf',
            call_id: oc.id,
            digits: '4321',
            mode: 1,
            media_id: 1
        },
    ], 2000)

    await sip.call.terminate(oc.id)

    await z.wait([
        {
            event: 'call_ended',
            call_id: oc.id,
        },
        {
            event: 'call_ended',
            call_id: ic.id,
        },
        {
            event: 'response',
            call_id: oc.id,
            method: 'BYE',
            msg: sip_msg({
                $rs: '200',
                $rr: 'OK',
            }),
        },
    ], 1000)

    console.log("Success")

    await sip.stop()
    process.exit(0)
}


test()
.catch(e => {
    console.error(e)
    process.exit(1)
})

