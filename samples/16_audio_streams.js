...
async function test() {
    ...
    const t1 = await sip.transport.create({address: "127.0.0.1", type: 'tcp'})
    const t2 = await sip.transport.create({address: "127.0.0.1", type: 'tcp'})
    ...
    const oc = await sip.call.create(t1.id, {from_uri: 'sip:alice@test.com', to_uri: `sip:bob@${t2.address}:${t2.port}`, media: 'audio,audio,audio,audio,audio,audio,audio,audio,audio,audio,audio,audio,audio,audio,audio,audio'})
